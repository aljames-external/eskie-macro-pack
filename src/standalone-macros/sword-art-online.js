...
if (eskieModule?.socketlib) {
    // Check if there are any active shatters playing on this token
    const shatters = token.document.getFlag('eskie-macros', 'sao-shatters') || {};
    const isPlaying = Object.keys(shatters).length > 0;

    if (isPlaying) {
        // Toggle the effect off
        await eskie.effect.swordArtOnlineDeath.stop(token);
    } else {
        // Play the synchronized death shatter effect
    ...
    // Standalone fallback if the module is NOT active (runs entirely locally on this client)
    ...
    if (isPlaying) {
        ...
        const sequence = new Sequence()
            ...
            .thenDo(async () => {
                ...
                // Create tiles locally (since we don't have socketlib, we just call createEmbeddedDocuments)
