...
async function play(token, config = {}) {
    ...
    if (overlay.applyPC || overlay.applyGM) {
        ...
        if (!overlay.applyGM && game.settings.get('sequencer', 'user-effect-opacity') === SEQUENCER_DEFAULT_OPACITY) {
            console.warn(`${MODULE_TLA} | Sequencer user-effect-opacity is set to default (50). This may cause the blurred vision effect to appear for GMs as well. Consider lowering this if this is not intended.`);
        }
