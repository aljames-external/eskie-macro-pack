...
async function create(token, config = {}) {
    ...
    seq.effect()
        ...
        .attachTo(token, { align: 'top-right', edge: 'outer', bindVisibility: false, offset: { x: -0.18, y: 0.18 }, gridUnits: true, bindRotation: false })
    ...
    seq.effect()
        ...
        .name(EFFECT_NAME)
        .atLocation(token)
        .attachTo(token, { align: 'top-right', edge: 'outer', bindVisibility: false, offset: { x: -0.2, y: 0.2 }, gridUnits: true, bindRotation: false })
    ...
    seq.effect()
        ...
        .name(EFFECT_NAME)
        .atLocation(token)
        .attachTo(token, { align: 'top-right', edge: 'outer', bindVisibility: false, offset: { x: 0.057, y: -0.025 }, gridUnits: true, bindRotation: false })
    ...
    seq.thenDo(async () => {
        ...
        while (Tagger.hasTags(token, CALL_TAG)) {
            ...
            await new Sequence()
                ...
                .atLocation(token)
                .attachTo(token, { align: 'top-right', edge: 'outer', bindVisibility: false, offset: { x: colOffset, y: rowOffset }, gridUnits: true, bindRotation: false })
                .size(0.015, { gridUnits: true })
                .aboveLighting()
                .duration(10000)
                .zIndex(2)
